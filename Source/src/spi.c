...
void spi_dma_init(void)
{
	...
	dma_is.PeriphInc = DMA_PINC_DISABLE;
	dma_is.MemInc = DMA_MINC_DISABLE;
	dma_is.PeriphDataAlignment = DMA_PDATAALIGN_BYTE;
	dma_is.MemDataAlignment = DMA_MDATAALIGN_BYTE;
	dma_is.Mode = DMA_NORMAL;
	dma_is.Priority = DMA_PRIORITY_HIGH;
	dma_is.FIFOMode = DMA_FIFOMODE_DISABLE;
	dma_is.FIFOThreshold = DMA_FIFO_THRESHOLD_FULL;
	dma_is.MemBurst = DMA_MBURST_SINGLE;
	dma_is.PeriphBurst = DMA_PBURST_SINGLE;

	DmaHandle.Init = dma_is;

	DmaHandle.Instance = DMA2_Stream5;



	int worked = (HAL_DMA_Init(&DmaHandle) == HAL_OK);
